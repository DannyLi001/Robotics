...
class IKSolver:
    def __init__(self, urdf_path: str, end_link_name: str, ee_offset: float = 0.0):
        """
        ...
        if self.ee_offset != 0.0:
            offset_link = pk.frame.Link(
                name="ee_offset_link",
                offset=pk.frame.Transform(rot=None, pos=torch.tensor([0.0, 0.0, self.ee_offset], device=self.device)),
                children=[]
            )
        ...
        self.active_joint_indices = list(range(self.pk_chain.num_joints))
