...
from ...config.config import get_arm_classes
...
class FrankaArm:
    def __init__(self, name: str):
        ...
        self.arm_cfg = arms[name]
        if self.arm_cfg.type != "franka":
            raise ValueError(f"{name} is not a franka arm.") 
        
        # Basic setup
        self.robot = Robot(self.arm_cfg.ip)
        self.gripper = Gripper(self.arm_cfg.ip)
        
        # Load config
        self.home_qpos = np.array(self.arm_cfg.home_qpos)
        # self.home_pose = self.arm_cfg.home_pose
        self.urdf_path = self.arm_cfg.urdf_path
        self.ee_link_name = self.arm_cfg.ee_link_name
        self.joint_limits = np.array([(float(a), float(b)) for a, b in self.arm_cfg.joint_limits], dtype=np.float32)
        self.joint_velocity_limit = np.array(self.arm_cfg.joint_velocity_limit, dtype=np.float32)
        self.joint_acceleration_limit = np.array(self.arm_cfg.joint_acceleration_limit, dtype=np.float32)
        self.joint_jerk_limit = np.array(self.arm_cfg.joint_jerk_limit, dtype=np.float32)
        self.velocity_factor = self.arm_cfg.velocity_factor
